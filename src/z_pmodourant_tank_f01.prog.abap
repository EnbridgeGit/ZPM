...
*&---------------------------------------------------------------------*
*& 2015/11/30  gymana     SDP90969     Modified select statement to get
*&                                     object number from table iflot
*&---------------------------------------------------------------------*
...
*   Title to ALV
  ...
  TRY.
      ...
      lv_ltext = 'Year Built'(042).
      lv_mtext = 'Year Built'(042).
      lv_stext = 'Yr.Built'(043).
      rf_column = rf_columns->get_column('YEAR_BUILT').
      rf_column->set_long_text( lv_ltext ).
      rf_column->set_medium_text( lv_mtext ).
      rf_column->set_short_text( lv_stext ).
      rf_column->set_visible( space ).


      lv_ltext = 'Thermal Relief Pressure'(044).
      lv_mtext = 'Ther. Rlf. Press.'(045).
      lv_stext = 'T.Rl.Pres'(046).
      rf_column = rf_columns->get_column('THERMAL_RELIEF_PRESS').
      rf_column->set_long_text( lv_ltext ).
      rf_column->set_medium_text( lv_mtext ).
      rf_column->set_short_text( lv_stext ).
      rf_column->set_visible( space ).

      lv_ltext = 'Thermal Relief Pressure UOM'(047).
      lv_mtext = 'TRlf.Press.UOM'(048).
      lv_stext = 'TRPrs UOM'(049).
      rf_column = rf_columns->get_column('THERMAL_RELIEF_PRESS_UOM').
      rf_column->set_long_text( lv_ltext ).
      rf_column->set_medium_text( lv_mtext ).
      rf_column->set_short_text( lv_stext ).
      rf_column->set_visible( space ).

      lv_ltext = 'CRN Number'(050).
      lv_mtext = 'CRN Number'(050).
      lv_stext = 'CRN Number'(050).
      rf_column = rf_columns->get_column('CRN_NUMBER').
      rf_column->set_long_text( lv_ltext ).
      rf_column->set_medium_text( lv_mtext ).
      rf_column->set_short_text( lv_stext ).
      rf_column->set_visible( space ).

      lv_ltext = 'Level Indicator Type'(051).
      lv_mtext = 'Level Ind. Type'(052).
      lv_stext = 'L.Ind.Type'(053).
      rf_column = rf_columns->get_column('LEVEL_INDICATOR_TYPE').
      rf_column->set_long_text( lv_ltext ).
      rf_column->set_medium_text( lv_mtext ).
      rf_column->set_short_text( lv_stext ).
      rf_column->set_visible( space ).

      lv_ltext = 'Thermal Relief'(054).
      lv_mtext = 'Thermal Relief'(054).
      lv_stext = 'Th. Relief'(055).
      rf_column = rf_columns->get_column('THERMAL_RELIEF_IND').
      rf_column->set_long_text( lv_ltext ).
      rf_column->set_medium_text( lv_mtext ).
      rf_column->set_short_text( lv_stext ).
      rf_column->set_visible( space ).
...
**--START OF CHANGES BY AKMADASU
  ...
  data:gt_KSSK_INOB type TABLE OF ty_inob1,
       gs_KSSK_INOB type ty_inob1,
       gt_equi TYPE TABLE OF ty_equi1,
       gs_equi type ty_equi1.
...
* BOI by PANUSURI Ticket ACR-4909
        wa_status          TYPE bapi_itob_status.
  CONSTANTS: lco_i0076     TYPE j_status VALUE 'I0076',
             lco_i0320     TYPE j_status VALUE 'I0320'.
* EOI by PANUSURI Ticket ACR-4909

  FIELD-SYMBOLS: <fs_equi> TYPE ty_equi,
                 <fs_inob> TYPE ty_inob.

*  SDP90969 - G. Ymana
*  Modified select statement to get object number from table iflot
*  This number will be used to get the measurement points and documents
**--**-- START OF CHANGES BY AKMADASU
  IF s_equnr IS INITIAL AND s_staton IS INITIAL.
    IF s_ttype IS INITIAL.
      MESSAGE 'Please enter Tank Type' TYPE 'I'.
      LEAVE LIST-PROCESSING.
    else.
      SELECT CLINT INOB_OBJEK OBTAB from KSSK_INOB into TABLE gt_KSSK_INOB
                               where CLINT eq '2484' and
                                     OBTAB EQ 'EQUI'.
      IF sy-subrc is INITIAL.
        LOOP AT  gt_KSSK_INOB into gs_KSSK_INOB.
*          s_equnr-low = gs_KSSK_INOB-INOB_OBJEK.
          R_equnr-sign = 'I'.
          R_equnr-option = 'EQ'.
          R_equnr-low = gs_KSSK_INOB-INOB_OBJEK.
          append R_equnr.
          clear:gs_KSSK_INOB.
        ENDLOOP.
...
* Get Equipments
        SELECT a~equnr
               ...
               FROM v_equi AS a INNER JOIN iflot AS b
                 ON a~tplnr = b~tplnr
               WHERE a~equnr in r_equnr  AND
                     a~eqart eq 'ME_ST'  and
                     a~spras EQ sy-langu AND
                     a~datbi GE p_date AND
                     a~datab LE p_date AND
                     a~stort IN s_staton.
        IF sy-subrc NE 0.
...
* BOI by PANUSURI Ticket ACR-4909
  IF ta_equi IS NOT INITIAL.
*   Exclude Equipments with status DLFL and INAC
    LOOP AT ta_equi ASSIGNING <fs_equi>.
      CALL FUNCTION 'BAPI_EQUI_GETSTATUS'
        EXPORTING
          equipment     = <fs_equi>-equnr
        TABLES
          system_status = ta_status
          user_status   = ta_ustatus.

      READ TABLE ta_status INTO wa_status WITH KEY status = lco_i0076.
      IF sy-subrc = 0.
        <fs_equi>-del = abap_true.
      ELSE.
        READ TABLE ta_status INTO wa_status WITH KEY status = lco_i0320.
        IF sy-subrc = 0.
          <fs_equi>-del = abap_true.
        ENDIF.
      ENDIF.
...
* Get Characteristics
  SELECT atinn
         ...
         WHERE atnam IN ('TANK_MEASUREMENT_UOM',
                         ...
                         'TANK_TYPE',
                          'YEAR_BUILT',
                          'THERMAL_RELIEF_PRESS',
                          'THERMAL_RELIEF_PRESS_UOM',
                          'CRN_NUMBER',
...
* Get Tank values
    PERFORM get_tank_values   USING wa_equi-equnr
                              ...
                              CHANGING wa_final-capacity
                                       ...
                                       wa_final-message
                                       wa_final-year_built
                                       wa_final-thermal_relief_press
                                       wa_final-thermal_relief_press_uom
                                       wa_final-crn_number
                                       wa_final-thermal_relief_ind
                                       wa_final-level_indicator_type.
...
FORM get_tank_values    USING    p_equnr TYPE equnr " Equipment Number
                        ...
                        CHANGING p_capacity TYPE any " Tank Capacity
                                 ...
                                 p_year_built    TYPE any
                                 p_thermal_relief_press TYPE any
                                 p_thermal_relief_press_uom TYPE atwrt
                                 p_crn_number   TYPE atwrt
                                 p_thermal_relief_ind TYPE atwrt
                                 p_level_indicator_type TYPE atwrt.
...
* Convert Measurement UOM to tank measurement UOM in Characteristic
      CASE  p_level_uom.
        ...
        WHEN 'PCT'.
          lv_height = ( p_left / 100 ) * lv_diameter.
          p_left = ( p_left / 100 ) * lv_diameter.
        WHEN OTHERS.
...
* Convert Volume to US GAL
  ...
  PERFORM get_characteristic_value USING p_equnr
                                     'YEAR_BUILT'
                                   CHANGING  p_year_built.
  PERFORM get_characteristic_value USING p_equnr
                                     'THERMAL_RELIEF_PRESS'
                                   CHANGING  p_thermal_relief_press.
  PERFORM get_characteristic_value USING p_equnr
                                     'THERMAL_RELIEF_PRESS_UOM'
                                   CHANGING  p_thermal_relief_press_uom.
  PERFORM get_characteristic_value USING p_equnr
                                     'CRN_NUMBER'
                                   CHANGING  p_crn_number.
  PERFORM get_characteristic_value USING p_equnr
                                     'LEVEL_INDICATOR_TYPE'
                                   CHANGING  p_level_indicator_type.
  PERFORM get_characteristic_value USING p_equnr
                                     'THERMAL_RELIEF_IND'
                                   CHANGING  p_thermal_relief_ind.
