...
FORM prepare .
  ...
  TYPES: BEGIN OF lty_level4,
           tplnr TYPE iflo-tplnr,
           stort TYPE iflo-stort,
  ...
  DATA: lt_logs     TYPE STANDARD TABLE OF zpmt_equip_log,
        ...
        lv_floc     TYPE tplnr,
        ls_level4   TYPE lty_level4,
        lt_level4   TYPE STANDARD TABLE OF lty_level4,
        ls_final    LIKE LINE OF gt_final,
        lv_value1   type z_varvaluel.
...
**fetch the function location level 3 based on premise and station id.
    SELECT tplnr stort msgrp FROM iflo INTO TABLE gt_iloa_e
                                  FOR ALL ENTRIES IN lt_logs
                                  WHERE eqart IN ('ST_RG', 'ST_TR', 'ST_MD') AND
                                        stort EQ lt_logs-stort AND
...
*take only level 3 th Level Floc
            ...
            IF sy-subrc EQ 0.
              ...
              ls_level4-tplnr = lv_floc.
              ls_level4-stort = <fs_logs>-stort.
              ls_level4-msgrp = <fs_logs>-premise.
              APPEND ls_level4 TO lt_level4.
              CLEAR ls_level4.
            ENDIF.
          ENDIF.
...
**fetch the all 5th level Floc(there will be multiple Flocation at level 5)
    IF lt_level4[] IS NOT INITIAL.
      SORT lt_level4 BY tplnr.
      LOOP AT gt_iflot ASSIGNING <fs_iflot>.
        CLEAR ls_level4.
        READ TABLE lt_level4 INTO ls_level4 WITH KEY tplnr = <fs_iflot>-tplma
                                            BINARY SEARCH.
        IF sy-subrc NE 0.
          <fs_iflot>-del = abap_true.
        ELSE.
          <fs_iflot>-stort = ls_level4-stort.
          <fs_iflot>-msgrp = ls_level4-msgrp.
        ENDIF.
      ENDLOOP.
  ...
  LOOP AT gt_logs ASSIGNING <fs_logs>.
    ...
    CASE <fs_logs>-proc_status.
      WHEN '1'.                                             "SDP86643
        ls_final-update_stat = 'Equipment Not In SAP'(056). "SDP86643
        gv_missing = gv_missing + 1.                        "SDP86643
      WHEN '2'.
      ...
      WHEN '4'.
        ls_final-update_stat = 'No changes'(006).           "SDP86643
        gv_created = gv_created + 1.
      WHEN '5'.
        ls_final-update_stat = 'Equipment Processed'(055).  "SDP86643
        gv_created = gv_created + 1.
    ENDCASE.
...
* SDP86643 - G. Ymana - Bypass records with no changes
    IF ls_final-update_stat = 'No changes'.
      CLEAR ls_final.
      CONTINUE.
    ENDIF.

* SDP86643 - G. Ymana - Bypass all records with an ICAT code found
* in table ZVAR

      SELECT value1 FROM ZVAR into lv_value1
       WHERE PROGRAMM EQ 'Z_PM_BANNER_INTERFACE_ANALYSIS'
         AND value1 EQ ls_final-banner_code.
      ENDSELECT.
